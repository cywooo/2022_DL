...
w2 = np.random.randn(Num_of_hiden1, 1)  #* np.sqrt(1. / 16)
b1 = np.random.randn(Num_of_hiden1,1)      #* np.sqrt(1. / 16)
b2 = np.random.randn(Num_of_outLayer,1)       #* np.sqrt(1. / 1)
...
def back( feature,real, w1, w2, b1, b2, l1, l2, a1, a2, rl,n): 
    ...
    dw1=np.dot(feature.T,(np.dot((w2*dactive(a1)),Edy.T*dactive(a2))).T)
...
plt.plot(np.linspace(1, train_num, train_num), l2_.T, label='predict')
plt.xlabel('# of Data')
plt.title('compare')
plt.legend(title='learning curve :')
plt.show()
...
plt.plot(np.linspace(1, valid_num, valid_num), l2_.T, label='predict')
plt.xlabel('# of Data')
plt.title('compare')
plt.legend(title='learning curve :')
plt.show()
